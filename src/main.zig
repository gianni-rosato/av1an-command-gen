...
pub fn main() !void {
    ...
    if (encoder_tgt == encoderTarget.aom) {
        ...
        try stdout.print("av1an -i \"INPUT.mkv\" --verbose --split-method av-scenechange -m lsmash -c mkvmerge --sc-downscale-height {d} -e aom --force -v \"--good --bit-depth=10 --tile-columns={d} --tile-rows={d} --end-usage=q --threads=2 --tune=ssim --ssim-rd-mult=125 --tune-content=psy --arnr-maxframes=15 --arnr-strength=2 --enable-cdef=0 --loopfilter-control=3 --quant-sharpness=3 --deltaq-mode=1 --aq-mode=0 --enable-keyframe-filtering=1 --luma-bias=25 --luma-bias-strength=15 --luma-bias-midpoint=66 --enable-qm=1 --quant-b-adapt=1 --lag-in-frames={d} --sb-size=dynamic --disable-kf --kf-max-dist=9999 --cpu-used={d} --cq-level={d} --denoise-noise-level=9 --enable-dnl-denoising=0\" --pix-format yuv420p10le -a \"-c:a libopus -b:a 128k -ac 2\" -x {d} --set-thread-affinity 2 -w 0 -o \"OUTPUT.mkv\"\n", .{ height, colsl, rowsl, lif, encoderSpeed, crf_aom, xs });
    } else if (encoder_tgt == encoderTarget.svt) {
        try stdout.print("Generated Command: ", .{});
        try stdout.print("av1an -i \"INPUT.mkv\" --verbose --split-method av-scenechange -m lsmash -c mkvmerge --sc-downscale-height {d} -e svt-av1 --force -v \"--tile-rows {d} --tile-columns {d} --input-depth 10 --tune 2 --enable-overlays 1 --enable-qm 1 --qm-min 0 --qm-max 15 --keyint -1 --scd 0 --lp 1 --irefresh-type 1 --crf {d} --preset {d} --film-grain 12 --film-grain-denoise 0\" --pix-format yuv420p10le -a \"-c:a libopus -b:a 128k -ac 2\" -x {d} --set-thread-affinity 2 -w 0 -o \"OUTPUT.mkv\"\n", .{ height, rowsl, colsl, crf_svt, encoderSpeed, xs });
    } else if (encoder_tgt == encoderTarget.rav1e) {
        try stdout.print("Generated Command: ", .{});
        try stdout.print("av1an -i \"INPUT.mkv\" --verbose --split-method av-scenechange -m lsmash -c mkvmerge --sc-downscale-height {d} -e rav1e --force -v \"--tile-rows {d} --tile-cols {d} --threads 2 --no-scene-detection -s {d} --quantizer {d} --photon-noise 13\" --pix-format yuv420p10le -a \"-c:a libopus -b:a 128k -ac 2\" -x {d} --set-thread-affinity 2 -w 0 -o \"OUTPUT.mkv\"", .{ height, rows, cols, encoderSpeed, q_rav1e, xs });
    }
}
